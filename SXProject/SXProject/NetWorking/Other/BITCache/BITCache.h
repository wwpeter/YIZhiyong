/**
 `BITCache` is a thread safe key/value store designed for persisting temporary objects that are expensive to
 reproduce, such as downloaded data or the results of slow processing. It is comprised of two self-similar
 stores, one in memory (<BITMemoryCache>) and one on disk (<BITDiskCache>).
 
 `BITCache` itself actually does very little; its main function is providing a front end for a common use case:
 ...
 repopulated in memory the next time they are accessed. `BITCache` also does the tedious work of creating a
 ...
 separately if necessary. See the docs for <BITMemoryCache> and <BITDiskCache> for more details.
 */
...
typedef void (^BITCacheBlock)(BITCache *cache);
typedef void (^BITCacheObjectBlock)(BITCache *cache, NSString *key, id object);

@interface BITCache : NSObject
...
/**
 The underlying disk cache, see <BITDiskCache> for additional configuration and trimming options.
 */
@property (readonly) BITDiskCache *diskCache;

/**
 The underlying memory cache, see <BITMemoryCache> for additional configuration and trimming options.
 */
@property (readonly) BITMemoryCache *memoryCache;
