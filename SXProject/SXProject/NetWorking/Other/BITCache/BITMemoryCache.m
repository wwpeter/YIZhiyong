#import "BITMemoryCache.h"

#if __IPHONE_OS_VERSION_MIN_REQUIRED >= __IPHONE_4_0
#import <UIKit/UIKit.h>
#endif

NSString * const BITMemoryCachePrefix = @"com.tumblr.BITMemoryCache";

@interface BITMemoryCache ()
...
{
    if (self = [super init]) {
        NSString *queueName = [[NSString alloc] initWithFormat:@"%@.%p", BITMemoryCachePrefix, self];
...
- (void)handleMemoryWarning
{
    ...
    __weak BITMemoryCache *weakSelf = self;
    
    dispatch_async(_queue, ^{
        BITMemoryCache *strongSelf = weakSelf;
        if (!strongSelf)
            return;
...
- (void)trimToAgeLimitRecursively
{
    ...
    dispatch_after(time, _queue, ^(void){
        ...
        __weak BITMemoryCache *weakSelf = strongSelf;
...
- (void)objectForKey:(NSString *)key block:(BITMemoryCacheObjectBlock)block
{
...
- (void)setObject:(id)object forKey:(NSString *)key block:(BITMemoryCacheObjectBlock)block
{
    [self setObject:object forKey:key withCost:0 block:block];
}

- (void)setObject:(id)object forKey:(NSString *)key withCost:(NSUInteger)cost block:(BITMemoryCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
...
- (void)removeObjectForKey:(NSString *)key block:(BITMemoryCacheObjectBlock)block
{
    if (!key)
        return;
...
- (void)trimToDate:(NSDate *)trimDate block:(BITMemoryCacheBlock)block
{
    if (!trimDate)
        return;
...
- (void)trimToCost:(NSUInteger)cost block:(BITMemoryCacheBlock)block
{
    __weak BITMemoryCache *weakSelf = self;
...
- (void)trimToCostByDate:(NSUInteger)cost block:(BITMemoryCacheBlock)block
{
    __weak BITMemoryCache *weakSelf = self;
...
- (void)removeAllObjects:(BITMemoryCacheBlock)block
{
    __weak BITMemoryCache *weakSelf = self;
...
- (void)enumerateObjectsWithBlock:(BITMemoryCacheObjectBlock)block completionBlock:(BITMemoryCacheBlock)completionBlock
{
    if (!block)
        return;
...
- (id)objectForKey:(NSString *)key
{
    ...
    [self objectForKey:key block:^(BITMemoryCache *cache, NSString *key, id object) {
        objectForKey = object;
...
- (void)setObject:(id)object forKey:(NSString *)key withCost:(NSUInteger)cost
{
    ...
    [self setObject:object forKey:key withCost:cost block:^(BITMemoryCache *cache, NSString *key, id object) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeObjectForKey:(NSString *)key
{
    ...
    [self removeObjectForKey:key block:^(BITMemoryCache *cache, NSString *key, id object) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToDate:(NSDate *)date
{
    ...
    [self trimToDate:date block:^(BITMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToCost:(NSUInteger)cost
{
    ...
    [self trimToCost:cost block:^(BITMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToCostByDate:(NSUInteger)cost
{
    ...
    [self trimToCostByDate:cost block:^(BITMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeAllObjects
{
    ...
    [self removeAllObjects:^(BITMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)enumerateObjectsWithBlock:(BITMemoryCacheObjectBlock)block
{
    if (!block)
        return;
    ...
    [self enumerateObjectsWithBlock:block completionBlock:^(BITMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (BITMemoryCacheObjectBlock)willAddObjectBlock
{
    __block BITMemoryCacheObjectBlock block = nil;
...
- (void)setWillAddObjectBlock:(BITMemoryCacheObjectBlock)block
{
    __weak BITMemoryCache *weakSelf = self;
...
- (BITMemoryCacheObjectBlock)willRemoveObjectBlock
{
    __block BITMemoryCacheObjectBlock block = nil;
...
- (void)setWillRemoveObjectBlock:(BITMemoryCacheObjectBlock)block
{
    __weak BITMemoryCache *weakSelf = self;
...
- (BITMemoryCacheBlock)willRemoveAllObjectsBlock
{
    __block BITMemoryCacheBlock block = nil;
...
- (void)setWillRemoveAllObjectsBlock:(BITMemoryCacheBlock)block
{
    __weak BITMemoryCache *weakSelf = self;
...
- (BITMemoryCacheObjectBlock)didAddObjectBlock
{
    __block BITMemoryCacheObjectBlock block = nil;
...
- (void)setDidAddObjectBlock:(BITMemoryCacheObjectBlock)block
{
    __weak BITMemoryCache *weakSelf = self;
...
- (BITMemoryCacheObjectBlock)didRemoveObjectBlock
{
    __block BITMemoryCacheObjectBlock block = nil;
...
- (void)setDidRemoveObjectBlock:(BITMemoryCacheObjectBlock)block
{
    __weak BITMemoryCache *weakSelf = self;
...
- (BITMemoryCacheBlock)didRemoveAllObjectsBlock
{
    __block BITMemoryCacheBlock block = nil;
...
- (void)setDidRemoveAllObjectsBlock:(BITMemoryCacheBlock)block
{
    __weak BITMemoryCache *weakSelf = self;
...
- (BITMemoryCacheBlock)didReceiveMemoryWarningBlock
{
    __block BITMemoryCacheBlock block = nil;
...
- (void)setDidReceiveMemoryWarningBlock:(BITMemoryCacheBlock)block
{
    __weak BITMemoryCache *weakSelf = self;
...
- (BITMemoryCacheBlock)didEnterBackgroundBlock
{
    __block BITMemoryCacheBlock block = nil;
...
- (void)setDidEnterBackgroundBlock:(BITMemoryCacheBlock)block
{
    __weak BITMemoryCache *weakSelf = self;
