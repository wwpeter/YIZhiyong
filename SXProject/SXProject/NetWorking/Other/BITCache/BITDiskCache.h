/**
 `BITDiskCache` is a thread safe key/value store backed by the file system. It accepts any object conforming
 ...
 The designated initializer for `BITDiskCache` is <initWithName:>. The <name> string is used to create a directory
 ...
 Because this cache is bound by disk I/O it can be much slower than <BITMemoryCache>, although values stored in
 `BITDiskCache` persist after application relaunch. Using <BITCache> is recommended over using `BITDiskCache`
...
@class BITDiskCache;
@protocol BITCacheBackgroundTaskManager;

typedef void (^BITDiskCacheBlock)(BITDiskCache *cache);
typedef void (^BITDiskCacheObjectBlock)(BITDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL);

@interface BITDiskCache : BITNSObject
...
/**
 The URL of the directory used by this cache, usually `Library/Caches/com.tumblr.BITDiskCache.(name)`
...
/**
 ...
 For example:
    ...
    dispatch_sync([BITDiskCache sharedQueue], ^{
        NSLog(@"accurate, unchanging byte count: %d", [[BITDiskCache sharedCache] byteCount]);
    });
 */
 ...
 */
@property (copy) BITDiskCacheObjectBlock willAddObjectBlock;

/**
 A block to be executed just before an object is removed from the cache. The queue waits during execution.
 */
@property (copy) BITDiskCacheObjectBlock willRemoveObjectBlock;
 ...
 */
- (void)trimToDate:(NSDate *)date block:(BITDiskCacheBlock)block;
...
/**
 ...
 @param block A block to be executed serially after the cache has been trimmed, or nil.
 */
- (void)trimToSize:(NSUInteger)byteCount block:(BITDiskCacheBlock)block;
...
/**
 ...
 @param block A block to be executed serially after the cache has been trimmed, or nil.
 */
- (void)trimToSizeByDate:(NSUInteger)byteCount block:(BITDiskCacheBlock)block;
...
/**
 Set a global manager to be used for setting up/tearing down any background tasks needed by BITCache.
